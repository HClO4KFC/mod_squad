...
from moelize import MoEShell
# class MTMobileViT(VisionTransformer):
class MTMobileViT(MobileViT):
    """ Vision Transformer with support for global average pooling
    """
    def __init__(
            self, img_types, **kwargs):
        super(MTMobileViT, self).__init__(**kwargs)
        # 设置基本参数
        ...
        self.type_to_channel = {
            'depth_euclidean':1, 'depth_zbuffer':1, 'edge_occlusion':1, 
            'edge_texture':1, 'keypoints2d':1, 'keypoints3d':1, 'normal':3, 
            'principal_curvature':2,  'reshading':3, 'rgb':3, 
            'segment_semantic':18, 'segment_unsup2d':1, 'segment_unsup25d':1
        }
        # 删除单任务头
        del self.head
        self.encoder_output_size = kwargs['encode_output_size']
        # 创建多任务头
        self.task_heads = []
            ...
            if 'class' in img_type:
                ...
                self.task_heads.append(
                    nn.Sequential(
                        ...
                        nn.Linear(self.encoder_output_size, class_num)
                    )
                )
            else:
                ...
                self.task_heads.append(
                    nn.Sequential(
                        nn.Conv2d(self.encoder_output_size, channel, kernel_size=1),
    ...
    def forward(self, x, task_rank=None, get_flop=False):
        ...
        if task_rank is None:
            ...
            for idx in range(len(self.task_heads)):
                ans.append(self.task_heads[idx](x))
        else:
            assert task_rank < len(self.task_heads) and task_rank >= 0, 'invalid task_rank'
            ans = self.task_heads[task_rank](x)
        if get_flop:
            return ans
        return x, z_loss
...
class MTMobileViTMoETaskGating(MTMobileViT):
    def __init__(self, img_types, num_tasks, num_experts, 
        ...
        super(MTMobileViTMoETaskGating, self).__init__(
            img_types, num_tasks=num_tasks, **kwargs)
        self.ismoe = True
        ...
        #         'ffd_heads': ffd_heads, 'ffd_noise': ffd_noise, 'num_tasks': num_tasks, 
        ...
        if self.ismoe:
            self.moe_target = {'BottleneckBlock', 'MobileVitBlock'}
            for stage_idx in range(len(self.stages)):
                stage = self.stages[stage_idx]
                for module_idx in range(len(stage)):
                    expert_base = stage[module_idx]
                    if type(expert_base).__name__ in self.moe_target:
                        self.stages[stage_idx][module_idx] = MoEShell(
                            num_tasks=num_tasks, num_experts=num_experts, 
                            original=expert_base, input_size=input_size, 
                            output_size=output_size, k=k, cvloss=0.1, switchloss=0.1, 
                            zloss=0.1, **kwargs)
        else:
            assert False, 'none-moe model isnt implemented yet'
...
#             # mlp_bh = 1 if blk.attn.num_experts > blk.attn.num_tasks else 0
#             mlp_bh = 0

#             num_attn_experts = blk.attn.num_tasks
#             if hasattr(blk.attn, 'num_experts'):
#                 if blk.attn.num_experts > blk.attn.num_tasks:
#                     mlp_bh = 1
#                     choose = (np.array(expert_usage[0]) > args.thresh / blk.attn.num_tasks)
#                     num_attn_experts = int(choose.sum())

#                     if num_attn_experts < blk.attn.num_tasks: # threshold too large
#                         ind = np.argpartition(np.array(expert_usage[0]), -blk.attn.num_tasks)[-blk.attn.num_tasks:]
#                         choose[ind] = True
#                         num_attn_experts = blk.attn.num_tasks
...
#                         dim=self.R['dim'], num_tasks=self.R['num_tasks'], mlp_ratio=self.R['mlp_ratio'], qkv_bias=self.R['qkv_bias'],
...
#                 select_id = (np.array(expert_usage[0]) > args.thresh / blk.attn.num_tasks)
...
#         patch_size=16, embed_dim=192, depth=12, num_tasks=3, mlp_ratio=4,
...
#     model = MTMobileViT(img_types,
#         patch_size=16, embed_dim=384, depth=12, num_tasks=6, mlp_ratio=4,
#         qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     return model

# def mt_m_vit_taskgate_mlp16E4_small(img_types, **kwargs): # 67.37M 5.21G
#     model = MTMobileViTMoETaskGating(img_types, 
#         patch_size=16, embed_dim=384, depth=12, num_tasks=6, mlp_ratio=4, qkv_bias=True,
...
#     model = MTMobileViTMoETaskGating(img_types, 
#         patch_size=16, embed_dim=768, depth=12, num_tasks=3, qkv_bias=True,
...
#     model = MTMobileViTMoETaskGating(img_types, 
#         patch_size=16, embed_dim=384, depth=12, num_tasks=6, qkv_bias=True,
...
#     model = MTMobileViTMoETaskGating(img_types, 
#         patch_size=16, embed_dim=768, depth=12, num_tasks=12, qkv_bias=True,
...
#     model = MTMobileViT(img_types,
#         patch_size=16, embed_dim=768, depth=12, num_tasks=12, mlp_ratio=4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     return model
...
#     model = MTMobileViT(img_types,
#         patch_size=16, embed_dim=1024, depth=24, num_tasks=16, mlp_ratio=4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     return model
...
#     model = MTMobileViT(img_types,
#         patch_size=14, embed_dim=1280, depth=32, num_tasks=16, mlp_ratio=4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     return model
...
if __name__ == '__main__':
    ...
    pretrained = False
    mbvit_version = 'mobilevit_xxs'
    encode_output_size = 512
    ...
    # }
    model = MTMobileViTMoETaskGating(
        img_types=img_types,
        num_tasks=num_tasks, 
        ...
        mbvit_version=mbvit_version, 
        encode_output_size=encode_output_size,
